...
class RuleEngine:
    ...
    _EXEC_BATCH = 500  # linhas processadas por lote (limite de memória)

    def execute_rule(self, rule: Dict, db, table_name: str,
                     row_ids: Optional[List[int]] = None
                     ) -> Tuple[int, List[str]]:
        """Executa a regra em lotes para limitar uso de memória.

        Processa _EXEC_BATCH linhas por vez:
          1. Lê um lote via fetchmany()
          2. Avalia condições e constrói updates
          3. Grava o lote em uma única transação
          4. Repete até acabar as linhas
        """
        ...
        # Abre cursor de leitura (mantido aberto entre os lotes)
        try:
            with db._lock:
                cur = db.conn.cursor()
                if row_ids:
        ...
        # Processa em lotes para não explodir memória em tabelas grandes
        while True:
            # ── Lê próximo lote ────────────────────────────────────────────
            try:
                with db._lock:
                    batch = cur.fetchmany(self._EXEC_BATCH)
            except Exception as exc:
                errors.append(f"Erro ao ler lote: {exc}")
                break
            if not batch:
                break

            # ── Avalia condições e monta updates ───────────────────────────
            ...
            for row_tuple in batch:
                ...
                for action in actions:
                    ...
                    if not field or field not in cols:
                        continue
                    try:
                        new_val = self._apply_action(
                            atype, action, field, row, errors, db)
                        if new_val is not None:
                            updates[field] = new_val
                            row[field] = new_val   # cascata dentro da linha
                    except Exception as exc:
                        errors.append(
                            f"Ação [{atype}] campo '{field}': {exc}")
                ...
                if updates:
                    ...
                    pending.append((
                        f'UPDATE "{table_name}" '
                        f'SET {set_clause} WHERE "{ROW_ID_COL}" = ?',
                        vals
                    ))

            # ── Grava lote em transação única ──────────────────────────────
            if pending:
                try:
                    with db._lock:
                    ...
                    try:
                        ...
                        pass
                    errors.append(f"Erro ao salvar lote: {exc}")
